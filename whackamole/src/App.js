...
const App = () => {
  ...
  const startGame = (timeToPlay, popMin, popMax, difficulty) => {
  ...
  const theTimer = (seconds, timeToPlay) => {
  ...
  const displayTimer = (seconds, timeToPlay) => {
  ...
  const updateLeaderboard = (name, score, difficulty) => {
  ...
  const updateStorage = (difficulty, currentList) => {
    if (currentList === [["", ""]]) return
    let asd = `${difficulty}List`
    localStorage.setItem(asd, JSON.stringify(currentList))
  }

  const displayLeaderboard(e, difficulty) {
  ...
  lbuttons.forEach(button =>
    button.addEventListener(
      "click",
      displayLeaderboard.bind(null, button.value)
    )
  )
